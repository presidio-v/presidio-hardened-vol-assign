...
% --- MDPI extended-version disclosure (mandatory for the SI) ---
\begin{center}
\small
\fbox{\parbox{0.95\textwidth}{
This paper is an extended version of Rabiei, P.; Arias-Aranda, D.; Stantchev, V.
\emph{A Novel Multi-Objective Optimization Model for Directing Affected People
to Relief Centers in Post-Disaster Scenarios: Combining Fuzzy Inference Systems
with NSGA-II and NRGA.} \emph{Autonomous Transportation Research}, in press,
2026 (ATRES-S-26-00021). The present paper extends that work by reframing the
model in supply-chain-resilience theory, splitting the transportation
infeasibility objective into separate Robustness and Rapidity components,
adding NSGA-III as a third evolutionary algorithm, introducing rule-base and
weight-perturbation sensitivity analyses, and releasing a Python reference
implementation as an open-source artifact.
}}
\end{center}
...
three-objective ATRes formulation fuses into one --- a visibility gain
for the decision-maker, established by rank-correlation analysis.
NSGA-III is 32\% faster than NSGA-II at every size with statistically
equivalent Pareto-front quality. Weight-perturbation sweeps (100
...
\texttt{github.com/presidio-v/presidio-hardened-vol-assign}.
\end{abstract}
...
of the four objectives are inherited from the recent ATRes line of work
\cite{rabiei2021, rabiei2023, atres2026}; the fourth is new and is the
result of splitting the prior work's transportation-infeasibility index
...
This paper makes five contributions to the literature on humanitarian
last-mile allocation and supply chain resilience:
\begin{enumerate}
    ...
    \item An empirical comparison of NSGA-II, NRGA, and NSGA-III on the
          four-objective humanitarian allocation problem
    ...
    \item An open-source Python reference implementation released with a
          ...
          line of work \textbf{[cite Zenodo DOI when minted]}.
\end{enumerate}
...
\cite{rabiei2023}. Both pair Mamdani FIS with NSGA-II and NRGA. The ATRes
manuscript \cite{atres2026} adapts the pattern to the people-to-relief-
centre allocation problem we extend here, introducing the three qualitative
indices (FLPP, TFL, CABL) and their corresponding objective functions
(ULPP, TIL, CAIL). Adjacent fuzzy or stochastic disaster-allocation work
...
the Transportation Infeasibility Level (TIL) in \cite{atres2026} fuses
...
papers we surveyed releases a reproducible artifact: ATRes uses MATLAB
R2024b without an accompanying implementation, and the prior papers in
the line do similarly. This combination
...
line \cite{rabiei2021, rabiei2023, atres2026} and we retain it for
continuity with that line.

The MDPI extended version's algorithmic contribution is to add NSGA-III
...
The combination of (i) FIS-evaluated qualitative objectives organised around
a named resilience framework, (ii) a four-objective formulation that
...
\cite{atres2026}. The constraint $n_{\mathrm{dir}} < m$ is operational, not
...
\subsection{Resilience-anchored qualitative indices}\label{sec:model-indices}
...
with four qualitative indices, each anchored in one component of Bruneau et
al.'s \cite{bruneau2003} 4R supply-chain-resilience framework. Three of the
indices appeared in the ATRes work \cite{atres2026}; the fourth and the
...
shares or proportional allocation. We follow ATRes \cite{atres2026} in
naming the index ``Fairness Level in People Prioritization''; readers
...
The ATRes paper folded transport robustness and rapidity together into a
...
Every notation that already appeared in ATRes \cite{atres2026} is preserved;
the additions for the four-objective formulation are flagged in the rightmost
column.

\begin{table}[!t]
\centering
\caption{Notation. Symbols carried over from \cite{atres2026} are unchanged;
new symbols for the four-objective formulation are marked.}
\label{tab:notation}
\small
...
\midrule
$P$, $m$ & Set of affected people; $m = |P|$. & ATRes \\
$C$, $n$ & Set of relief centres; $n = |C|$. & ATRes \\
$n_{\mathrm{dir}}$ & Number of people we direct ($0 < n_{\mathrm{dir}} < m$). & ATRes \\
$d_{j,i}$ & Binary indicator: $d_{j,i} = 1$ iff person $j$ directed to centre $i$. & ATRes \\
$\mathrm{VS}_j \in [0,1]$ & Vulnerability score of person $j$. & ATRes \\
$\mathrm{IDL}_j \in [0,100]$ & Infrastructure damage at person $j$'s location. & ATRes \\
$\mathrm{RTR}_j$ (hours) & Resource time remaining for person $j$. & ATRes \\
$\mathrm{COR}_i \in [0,100]$ & Centre $i$ occupancy rate. & ATRes \\
$\mathrm{RDR}_i \in [0,100]$ & Centre $i$ resource depletion rate. & ATRes \\
$\mathrm{TD}_{j,i}$ (min) & Travel duration $j \to i$. & ATRes \\
$\mathrm{RCS}_{j,i}$ & Road condition $\in \{0.1, 0.5, 1.0\}$. & ATRes \\
$\mathrm{PHS}_{j,i}$ & Possible-hazard score $\in \{0.05, 0.25, 0.5, 0.75, 1.0\}$. & ATRes \\
$\mathrm{RWS}_{j,i} \in [0,1]$ & Roadworthiness (high = safe), Eq.~\eqref{eq:rws}. & ATRes (sign-corrected) \\
$\mathrm{ULPP}_j \in [0,100]$ & Unfairness in prioritising person $j$. & ATRes \\
$\mathrm{TIL}_{j,i} \in [0,100]$ & Transport infeasibility (3-obj only). & ATRes \\
$\mathrm{TRD}_{j,i} \in [0,100]$ & Transport Robustness Deficit. & \textbf{new} \\
$\mathrm{RPD}_{j,i} \in [0,100]$ & Rapidity Deficit. & \textbf{new} \\
$\mathrm{CAIL}_{i,td}$ & Centre allocation imbalance contribution. & ATRes \\
...
\paragraph{Note on Eq.~\eqref{eq:rws}: sign correction relative to ATRes.}
ATRes \cite{atres2026} defines RWS as the bare weighted average of RCS and
PHS, which evaluate to higher values for worse conditions (Blocked $= 1.0$,
Extreme $= 1.0$). ATRes Table~3 then labels the result with the opposite
convention (\emph{High} RWS reads as ``safe and efficient routes''), and
ATRes Table~5 makes that the operative reading: $\mathrm{TIL}$ is low when
RWS is high. The two readings cannot both hold for the formula as written;
either the labels or the formula must invert. We invert the formula. The
sign-corrected form, Eq.~\eqref{eq:rws}, gives $\mathrm{RWS} = 1$ for a
perfectly safe and clear route and $\mathrm{RWS} = 0$ for an impassable one,
matching ATRes Tables~3 and~5 throughout. The present authors include the
lead and corresponding authors of \cite{atres2026}; we record the
correction here explicitly so that any work citing both papers can adopt
the corrected form unambiguously, and we will request a corresponding
amendment at proof stage of \cite{atres2026} if galley scheduling
permits.

\subsection{Objective functions}\label{sec:model-objectives}

The model minimises four mean-objective functions over the directed set:
\begin{align}
...
collects the per-allocation CAIL contributions at centre $i$. The constraints
follow ATRes \cite{atres2026}:
\begin{equation}
...
For the legacy three-objective formulation (used in
...
$\overline{\mathrm{TIL}}$ objective using the fused FIS$_2$ pathway from
ATRes \cite{atres2026}. Everything else is identical.

\subsection{Resilience-theoretic mapping (new)}\label{sec:resilience-mapping}

The 4R framework is not decoration. It defines what each objective is
\emph{trying to capture} and tells the practitioner how to read the
Pareto front. Table~\ref{tab:4r-mapping} states the bijection.
...
The framework is not a label we apply at the end. It is a discipline
we apply throughout: each FIS's input set, rule base, and output range
must be explicable in the vocabulary of exactly one 4R component, and
each 4R component must be operationalised by exactly one FIS. The
derivation above shows the discipline holding. Sections that follow
draw on three theoretical consequences of this discipline, stated next.
...
Resourcefulness into Rapidity --- the 4R framework forbids it, so we leave
RTR out. The framework is not a label we apply at the end; it is a discipline
we apply throughout.
...
throughout, for the same reason the original ATRes work did: linguistic
...
for CAIL); the legacy three-objective formulation reuses ATRes's FIS$_2$ for
...
Each individual is a flat list of length $2 n_{\mathrm{dir}}$. The first
$n_{\mathrm{dir}}$ entries are person indices, sampled without replacement
from $\{0, \ldots, m-1\}$ --- a partial permutation of the candidate
population. The remaining $n_{\mathrm{dir}}$ entries are real values
$r_k \in [0, 1)$, each decoded to a centre index via
$\lfloor r_k \cdot n \rfloor$ \cite{atres2026}. The encoding is a flattened
form of the two-row matrix used in ATRes; the two representations carry
...
deferred it to a single supplementary ablation to be run only if H2b
flagged boundary diversity as a problem (it did not; see
Section~\ref{sec:perf-algorithms}).
...
medium (8/225/75), and large (10/300/100). Small and large match ATRes's
test instances \cite{atres2026}; medium is new and added to give scaling
...
crossover probability $0.7$, mutation probability $0.2$ --- ATRes baselines
\cite{atres2026} held fixed across the matrix so the comparison is on
...
non-parametric Mann--Whitney $U$ otherwise. We report $p$-values under
...
our encoding.} ATRes \cite{atres2026} reports NSGA-II faster than NRGA on
the large instance. We replicate this comparison and find the opposite.
...
$p < 10^{-4}$). We do not claim to know the cause. ATRes implements the
model in MATLAB R2024b with the two-row matrix encoding of its Figure~2;
...
and platform/runtime overheads. Without re-implementing ATRes's exact
...
of \cite{atres2026}. The substantive lesson is that NSGA-II-vs-NRGA CPU
rankings on humanitarian allocation problems do not transfer between
implementations without verification. For the MDPI extended version's
narrative, the NSGA-II/NRGA ranking is replaced by the more robust
...
our own data without depending on ATRes-vs-our-Python comparison.
...
test statistics. Across all five quality metrics on the four-objective
formulation, NSGA-II and NSGA-III achieve identical NNS (100 of 100,
i.e.\ every individual on the boundary front), within-noise MID and SM,
significantly different HV (NSGA-II ahead), and significantly different CPU
time (NSGA-III ahead). NRGA loses 0--1\,\% of NNS to its uniform-random
tie-breaker, has the lowest HV at every size, and sits between the other
two on CPU time. The ranking is stable across all three problem sizes and
across both formulations.

\begin{table}[!t]
\centering
\caption{Mean $\pm$ s.d.\ over 30 reps per (size, algorithm) cell on the
four-objective formulation. HV is in millions; CPU time in seconds. The
$p$-column reports the Mann--Whitney $U$ two-sided test against NSGA-II.}
\label{tab:stats-summary}
\small
\begin{tabular}{l l r r r r r}
\toprule
Size & Algorithm & NNS & HV ($\times 10^6$) & MID & SM & CPU (s) \\
\midrule
\multirow{3}{*}{small}
  & NSGA-II  & $100.0$ & $21.08 \pm 0.47$ & $77.7 \pm 0.5$ & $0.81 \pm 0.06$ & $1.45 \pm 0.07$ \\
  & NRGA     & $99.1$  & $20.21 \pm 0.87$ & $77.4 \pm 0.5$ & $0.79 \pm 0.05$ & $1.16 \pm 0.06$ \\
  & NSGA-III & $100.0$ & $20.74 \pm 0.62$ & $77.7 \pm 0.5$ & $0.81 \pm 0.06$ & $0.91 \pm 0.07$ \\
\midrule
\multirow{3}{*}{medium}
  & NSGA-II  & $100.0$ & $18.81 \pm 0.40$ & $69.5 \pm 0.4$ & $0.78 \pm 0.05$ & $1.69 \pm 0.06$ \\
  & NRGA     & $99.9$  & $17.35 \pm 0.55$ & $69.3 \pm 0.4$ & $0.77 \pm 0.04$ & $1.40 \pm 0.05$ \\
  & NSGA-III & $100.0$ & $18.33 \pm 0.69$ & $69.5 \pm 0.4$ & $0.79 \pm 0.05$ & $1.15 \pm 0.06$ \\
\midrule
\multirow{3}{*}{large}
  & NSGA-II  & $100.0$ & $13.98 \pm 0.29$ & $58.6 \pm 0.4$ & $0.81 \pm 0.05$ & $1.99 \pm 0.05$ \\
  & NRGA     & $99.8$  & $12.86 \pm 0.29$ & $58.6 \pm 0.4$ & $0.81 \pm 0.04$ & $1.69 \pm 0.05$ \\
  & NSGA-III & $100.0$ & $13.59 \pm 0.33$ & $58.6 \pm 0.4$ & $0.81 \pm 0.05$ & $1.45 \pm 0.05$ \\
\bottomrule
\end{tabular}
\end{table}

The MID and SM values are nearly identical across algorithms within each
size, and the small differences are not statistically significant. We report
them for completeness; the binding metrics for the H2 verdict are HV and CPU
time, where the algorithms differ in opposite directions.
...
theoretical work. The H1 hypothesis tests whether splitting ATRes's TIL
...
space using ATRes's TIL fusion?

For each 4-obj front, we re-evaluate every (person, centre) allocation
through the legacy FIS$_2$ pathway --- compute the sign-corrected RWS by
...
ATRes RWS-weighted fusion. To separate the geometric component from the
...
against which the ATRes-fusion fraction is compared.

The result tempers the 50\% headline. Pooled across all 270 fronts, the
ATRes-fusion mean dominance fraction is 0.486; the null-distribution
mean is 0.479. The difference is $+0.008$ --- under one percentage point.
The mean z-score of the ATRes fraction in the per-front null distribution
is $+0.09$; only $7.4\%$ of fronts have an ATRes fraction above the
...
Spearman result establishes that --- but the specific 3-obj fusion that
ATRes uses is not worse than a random fusion at preserving them; it is
...
The 50\% dominance fraction we observe under the ATRes-RWS fusion is
...
property of projecting from 4D to 3D, not of ATRes's particular
...
to RWS feeding the legacy FIS$_2$ pathway, although that pathway is not
...
cause undetermined. The asymmetry is not a problem; the qualified
verdicts are the substance of an honest contribution. Three threads of
implication follow.
...
(Section~\ref{sec:h1-projection}). The split is not a methodological
luxury; it surfaces a real Robustness/Rapidity trade-off that practitioners
articulate and policy documents reference, and that disappears under
single-axis fusion. This is the first published evidence we are aware of
...
it produces a \emph{Pareto-front-equivalent recommendation} 30\% faster
than NSGA-II at every problem size we tested
...
The H4 inversion is the methodological reminder. ATRes \cite{atres2026}
reports NSGA-II faster than NRGA on the large instance; we replicate the
comparison and observe the opposite. We do not isolate the cause: the
ATRes pipeline (MATLAB R2024b, two-row encoding, MATLAB-default
\texttt{gamultiobj} operators) and our pipeline (Python, flattened
encoding, custom DEAP operators) differ along multiple axes
simultaneously, and we did not re-implement the ATRes operators to
...
from \texttt{github.com/presidio-v/presidio-hardened-vol-assign} at tag
\texttt{v0.2.0}, archived on Zenodo with a citable DOI. The Python CLI
...
work in this line \cite{rabiei2021, rabiei2023, atres2026} uses MATLAB
...
those carried over from \cite{atres2026} for FIS$_1$ and FIS$_3$ and
...
without losing the 4R-axis discipline. ATRes \cite{atres2026} explicitly
...
30\% CPU advantage at unchanged Pareto-front quality),
...
\texttt{github.com/presidio-v/presidio-hardened-vol-assign} (Zenodo DOI
to be minted at the v0.2.0 tag).
...
at \texttt{github.com/presidio-v/presidio-hardened-vol-assign}, tag
\texttt{v0.2.0}, archived on Zenodo (\textbf{[insert DOI]}).

% =====================================================================
\section*{Author Contributions}
\textbf{[TIER 2 placeholder — CRediT taxonomy.]}

\section*{Funding}
\textbf{[TIER 2 placeholder.]}
...
\textbf{[TIER 2 placeholder — Tables A.1–A.4 enumerating rules of FIS$_1$,
FIS$_{2a}$, FIS$_{2b}$, FIS$_3$.]}

\section{Statistical test outputs (extended)}\label{app:stats}
\textbf{[TIER 2 placeholder — full Mann--Whitney U / Independent-samples
T outputs for every (size, metric) cell.]}
