...
where turbulence hits other inputs the gap narrows, and on realised quality the crisp baseline
can be the more fragile of the two. We argue this localised fragility
...
resource-allocation decision under an information-degrading shock. We engage this venue's
economic-turbulence setting by analogy, mapping a relief demand shock to an economic one,
rather than with market data (Section~\ref{sec:discussion}). The system builds on the
...
\emph{Fuzzy inference under uncertainty.} Mamdani fuzzy inference
systems~\cite{zadeh1965,mamdani1975} encode expert judgement as linguistic if--then rules over
graded membership functions, and are attractive precisely when inputs are imprecise or
partially known --- as situational data is during a shock. Combined with multi-objective evolutionary optimisation, they have been applied to
...
\emph{Multi-objective evolutionary optimisation.} Algorithms such as NSGA-II~\cite{deb2002}
and NRGA~\cite{aljadaan2008} return a Pareto front of non-dominated trade-offs rather than a
single optimum, leaving the final commitment to a decision-maker. This is a strength for transparency but, as we show, a source
...
\emph{Decision stability, reproducibility, and operability.} In our reading, robustness of
multi-objective methods is more often examined against algorithmic stochasticity or against
perturbation of elicited preferences --- the route the companion study~\cite{paperA2026}
takes --- than against degradation of the \emph{input data} itself. Far less
attention is paid to stability of the committed decision under such degradation, or to whether the running system is bit-for-bit reproducible and auditable ---
...
to obtain a per-realisation \emph{sensitivity slope}, and test the fuzzy-vs-crisp slopes with a
paired Wilcoxon test; because allocation churn saturates at the first turbulence level
(Section~\ref{sec:rq1}), its slope is a summary statistic, not a dose-response gradient. The crisp baseline (Section~\ref{sec:system}) decides by a deterministic
...
target OS/Python builds, run by a continuous-integration matrix over \{Ubuntu, macOS\}
...
metric on the turbulence level to obtain a per-realisation \emph{sensitivity slope};
Table~\ref{tab:rq1}
...
\begin{table}[H]
\centering
\caption{RQ1 per-realisation sensitivity slopes (stability metric regressed on turbulence
level), fuzzy-MOEA vs.\ crisp baseline, small instance, twelve realisations per cell.
Allocation churn saturates at the first level, so its slope summarises a step rather than a
gradient (see text). $p$ is a paired Wilcoxon test of the slopes.\label{tab:rq1}}
\small
...
\textbf{The decision collapses at the first perturbation.} Allocation churn is not a gradient
but a step: at the smallest turbulence we apply (level $0.05$) the fuzzy-MOEA already re-routes
$\approx 83\%$ of the directed people, and the magnitude is flat through the highest level
(Figure~\ref{fig:degradation-idl}, right). On the small instance this near-total churn occurs
in \emph{all eight} cells --- the churn fragility is close to universal there --- while the
crisp baseline re-routes far less (churn plateau $\approx 0.16$--$0.78$). The per-realisation
``slopes'' in Table~\ref{tab:rq1} therefore summarise a saturating step, not a dose response;
we use them for the paired test but read the effect as an immediate collapse, which is arguably
worse for trust than a gradual one. At scale the churn gap \emph{localises}: for the
person-priority inputs the model's objectives depend on it stays sharp
(Table~\ref{tab:rq1-large}: IDL noise and missingness, fuzzy $\approx 1.4$ vs.\ crisp
$0.16$--$0.37$, $p = 0.008$), but for centre-occupancy noise the crisp baseline saturates too
and the gap vanishes (fuzzy $1.45$ vs.\ crisp $1.44$; $p = 0.95$).
...
(fuzzy $8.5$ vs.\ crisp $23.4$; $p = 0.016$; road-condition flip is similar). Unlike the
near-universal churn collapse, then, the realised-quality harm is specific to the inputs the
MOEA objectives consume, and for the remaining centre/transport inputs the crisp baseline is
...
field/mode cell (small instance). The fuzzy-MOEA re-routes $\approx 80\%$ of the allocation in
every cell; the crisp baseline re-routes far less. At scale this gap narrows for the
centre/transport inputs (Table~\ref{tab:rq1-large}).\label{fig:churn}}
\end{figure}
...
degradation. The committed allocation \emph{collapses} --- most of it is re-routed --- at the
first whiff of input turbulence, near-universally on the small instance. What is
\emph{localised} is the harm: realised-objective quality degrades far more than a crisp
baseline only for the inputs the objectives consume (person priority, travel), and only at
scale does even the churn gap narrow to those inputs, the crisp baseline saturating on the
rest. Either way, a system that re-commits to a very different allocation under imperceptible
input noise cannot be trusted from a single run; that is precisely what makes the
reproducibility and auditability guarantees of Section~\ref{sec:rq2} load-bearing.
...
cross-environment reproducibility is then established by running the driver on each target OS /
Python in a continuous-integration matrix over \{Ubuntu, macOS\} $\times$ \{Python 3.11, 3.12\}
and diffing the signature column. Across those four environments every (size, seed) produced
the identical signature (cross-environment REP $= 1.0$ over five seeds): the decision is
bit-for-bit reproducible across platforms and Python versions, not merely within one. Together
with the run manifest, the dependency audit, and the
...
a laptop without specialised infrastructure.
...
subset. Cross-environment reproducibility is demonstrated across four environments on the small
instance and reduced solver budget; a sweep over more platforms and the larger instance remains
future work. These bound the claims to
