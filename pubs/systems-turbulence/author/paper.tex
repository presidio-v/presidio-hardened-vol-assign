...
(Results are on the small instance; a large-instance subset run is in progress to confirm
the effect holds at scale.)
...
\textbf{Decision churn is universally higher for the fuzzy-MOEA.} Across all eight
field/mode cells the fuzzy system's allocation-churn slope exceeds the crisp baseline's,
significantly in every case ($p \le 0.0034$; Figure~\ref{fig:churn}). Modest input
degradation re-routes a large fraction of the directed people --- the MOEA does not
commit to a stable allocation when its inputs are perturbed, regardless of which input is
perturbed or how.

\textbf{Realised-objective quality degrades more selectively.} The churn only translates
into worse realised objectives for perturbations of the inputs that feed the
person-priority and travel objectives (IDL, resource-time, travel-duration: fuzzy drift
slope $2$--$3\times$ the crisp baseline, all $p \le 0.0005$; Figure~\ref{fig:degradation-idl}).
For centre-occupancy noise and road-condition flips the crisp baseline degrades at least
as much, so the objective-quality gap closes even as the churn gap remains.
...
The finding inverts our starting hypothesis: fuzzy expert encoding does not buy graceful
degradation --- the MOEA decision is markedly \emph{input-fragile}, most sharply in the
decision itself (churn) and, for objective-feeding inputs, in realised quality too. Even
where quality holds, a system that re-commits to a very different allocation under
imperceptible input noise cannot be trusted from a single run; that is precisely what makes
the reproducibility and auditability guarantees of Section~\ref{sec:rq2} load-bearing.
